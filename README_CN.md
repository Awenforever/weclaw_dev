...
| 版本 | 影响对象 | 此前行为 | 当前行为 |
| --- | --- | --- | --- |
| v0.1.9-alpha | CoDeepSeedeX telemetry集成 | `/effort`和`/status`只能依赖部分本地状态或占位字段。 | `/effort max`使用dsproxy profile契约。`/status`读取dsproxy WeClaw telemetry，并展示model、effort、token context、token usage、`Details`、`Cost`、balance、pricing来源和更新时间、compaction策略、Compact/Trim、proxy和运行路径。使用CoDeepSeedeX集成时要求CoDeepSeedeX >= v0.3.9-alpha。 |
...
| 未发布main | 微信端slash commands | 命令回复混合语言，部分输出重复字段或表格过长。 | slash command回复统一为英文，并采用紧凑Markdown面板。`/status`显示profile、model、session、context、tokens、cost占位和当前余额。`/balance`使用固定宽度且仅显示Total的面板。`/info`显示WeClaw和dsproxy版本及uptime。 |
