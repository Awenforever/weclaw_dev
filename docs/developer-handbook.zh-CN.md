...
- 当前内部开发标签：`p0.1.5a36-alpha-prerelease-cleanup`
- 当前`main`和`origin/main`：合并后与`p0.1.5a36-alpha-prerelease-cleanup`指向同一提交
- 当前Release资产/代码线：规范镜像tag `p0.1.5a28-stop-semantics`和历史tag `v0.1p5a28-stop-semantics`均指向`31fa432`
- p5a29同步文档和Release note，p5a30删除过时文档入口，p5a31修复最终文档入口引用，p5a32同步手册到当前main线，p0.1.5a33确立规范化内部版本/tag格式，p0.1.5a34增加防回流测试，p0.1.5a35为42个可自动映射的历史`v0.1p...`内部tag创建规范镜像tag，p0.1.5a36删除了13个已验证存在规范镜像的历史`alpha-work-v0.1p...`临时pre-release/tag。这些文档类或tag镜像提交都不应与Release资产提交混淆。
...
- p0.1.5a36之后，内部开发使用规范化`p*.*.*`tag。历史`v0.1p...`tag仅作为追溯镜像保留，除非明确批准清理。历史`alpha-work-v0.1p...`临时pre-release/tag已在镜像验证后删除。
