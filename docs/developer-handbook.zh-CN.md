...
- p0.1.5a37之后，内部开发使用规范化`p*.*.*`tag。CI不再自动发布分支/tag pre-release。公开Release必须通过手动`release.yml`流程发布。历史`v0.1p...`tag仅作为追溯镜像保留，除非明确批准清理。
