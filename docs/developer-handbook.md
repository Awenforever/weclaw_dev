...
- After p0.1.5a37, use normalized `p*.*.*` internal tags. CI no longer auto-publishes branch/tag pre-releases. Public Releases must go through the manual `release.yml` workflow. Legacy `v0.1p...` tags remain only as historical mirrors until explicit cleanup is approved.
