...
- Current internal development tag: `p0.1.5a58-status-telemetry-polish`
- Last audited handoff baseline before this sync: `main=origin/main=p0.1.5a57-dsproxy-telemetry-contract=a3c53bc`
- Current active development line: `p0.1.5a58-status-telemetry-polish`. Resolve its exact commit from Git instead of trusting a copied static hash.
...
| Telemetry display quality | Mobile WeChat output remains compact and Markdown-first while showing model, effort, context window, token usage, estimated cost, balance, and compaction from `dsproxy`. | WeClaw `p0.1.5a58-status-telemetry-polish` | in_progress | 2026-05-16 | Context source, model-conflict diagnostics, paths, and missing-reason internals are hidden from compact `/status`. |
...
weclaw internal version: p0.1.5a58-status-telemetry-polish | <current-commit>
```
