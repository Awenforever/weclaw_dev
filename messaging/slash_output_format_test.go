...
func TestCompactContextPanelDoesNotExposeSourceOrMetricTable(t *testing.T) {
	...
	for _, want := range []string{"Context", "Tokens", "Cost     last~n/a  session~n/a", "￥12.34", "Proxy    thinking · 127.0.0.1:8001 · reachable", "Paths    cfg ~/.weclaw/config.json · log ~/.weclaw/weclaw.log"} {
		if !strings.Contains(got, want) {
