...
func TestBalanceReplyUsesSingleTableAndHidesEmptyGranted(t *testing.T) {
	got := formatBalanceReply(`{"status":"ok","balance":{"is_available":true,"balance_infos":[{"currency":"CNY","total_balance":"12.34","granted_balance":"","topped_up_balance":""}]}}`)
	for _, forbidden := range []string{"- CNY total:", "- CNY granted:", "- CNY topped-up:", "Granted", "Topped-up"} {
		if strings.Contains(got, forbidden) {
			t.Fatalf("balance reply contains duplicate or empty column token %q in:\n%s", forbidden, got)
		}
	}
...
func TestCompactContextPanelDoesNotExposeSourceOrMetricTable(t *testing.T) {
	got := strings.Join(buildCompactStatusPanel(nil, "user-1", 1000000, "thinking", "127.0.0.1:8001", "reachable"), "\n")
	for _, forbidden := range []string{"source:", "Metric", "Limit", "Used", "Left", "tools", "other"} {
		if strings.Contains(got, forbidden) {
			t.Fatalf("compact status panel contains old verbose token %q in:\n%s", forbidden, got)
		}
	}
	for _, want := range []string{"Context", "Tokens", "Cost     session n/a  last n/a", "Proxy    thinking · 127.0.0.1:8001 · reachable"} {
		if !strings.Contains(got, want) {
...
func TestSlashCommandOutputPreviewSnapshot(t *testing.T) {
	...
	preview := strings.Join([]string{
		...
		"",
		"## /status panel core",
		strings.Join(append([]string{"## 🧩 Status", "- **Profile:** `deepseek-thinking` @ACP", "- **Model:** `deepseek-v4-flash` `high`", "- **Session:** `thread-preview`", ""}, visualCommandFence("", buildCompactStatusPanel(nil, "user-1", 1000000, "thinking", "127.0.0.1:8001", "reachable")...)...), "\n"),
		"",
		"## /balance",
